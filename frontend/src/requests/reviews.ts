...
const createCreateReviewMutation = (movieid: number, text: string, numstars: number, username: string) => `mutation {
  createReview(movieid:${movieid},text:"${text}",numstars:${numstars},username:"${username}")
}`

export const createReview = (movieid: number, text: string, numstars: number, username: string): Promise<boolean> => {
  const body = {
